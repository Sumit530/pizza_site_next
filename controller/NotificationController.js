...
exports.follower_notification_list = async(req,res) =>{
    ...
    var today_followers_notification_data  = await Notification.find({receiver_id:req.body.user_id,type:3,createdAt:{ "$gte": new Date(moment().subtract(1, "days")),"$lte": new Date()}}).populate("user_id").sort({createdAt:-1})
    
    var today_data = []
    var yesterday_data = []
    if(today_followers_notification_data?.length > 0){
        var array_list = today_followers_notification_data?.map(async(e)=>{
   ...
   const result = await Promise.all(array_list)
...
exports.mentions_notification_list = async(req,res) =>{
    ...
    var mention_data  = await Notification.find({receiver_id:req.body.user_id,type:4,createdAt:{ "$gte": new Date(moment().subtract(1, "days")),"$lte": new Date()}}).populate("video_id").populate("user_id").sort({createdAt:-1})
    var today_data = []
    var yesterday_data = []
...
exports.comment_notification_list = async(req,res) =>{
    ...
    var mention_data  = await Notification.find({receiver_id:req.body.user_id,type:2,createdAt:{ "$gte": new Date(moment().subtract(1, "days")),"$lte": new Date()}}).populate("video_id").populate("user_id").sort({createdAt:-1})
    var today_data = []
    var yesterday_data = []
    if(mention_data.length > 0){
        ...
        const result = await Promise.all(mentionList)
